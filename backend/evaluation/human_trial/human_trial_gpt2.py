...
if __name__ == "__main__":
  ...
  prevConcepts = ["today"]
  input_ids_history = []
  input_ids_history.append([s2_id] + tokenizer.encode(initial_prompt))

  conversation_history = []
  conversation_history = ["GPT2: " + initial_prompt]
  targets_list = []
  emotion_scores_list = []
  user_concepts_list = []
  ...
  while i <= 20:
    ...
    conversation_history.append("GPT2 Bot: " + response)
  ...
  i = min(i, 20)
  print(f"Bot 1: Are you feeling the emotion {kw_emotion} ({emotional_kw})? target: {max_emotion}")
  print(f"Number of turns: {i}")
  print("Please complete survey at: https://imperial.eu.qualtrics.com/jfe/form/SV_6gtGIp4WoBpD6iq")
